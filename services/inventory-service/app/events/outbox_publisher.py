...
def publish_to_dlq(event: OutboxEvent, error: Exception):
    ...
    print(
        f"[Inventory Outbox Publisher] Event sent to DLQ. "
        ...
        try:
            ...
            for event in events:
                try:
                    ...
                    print(
                        f"[Inventory Outbox Publisher] Published "
                    ...
                    if event.retry_count >= MAX_RETRY_COUNT:
                        try:
                            ...
                            print(
                                f"[Inventory Outbox Publisher] DLQ publish failed. "
                        ...
                        print(
                            f"[Inventory Outbox Publisher] Event moved to failed state. "
                            f"outbox_id={event.id} retry_count={event.retry_count}"
                        )

                    else:
                        print(
                            f"[Inventory Outbox Publisher] Publish failed. "
