...
def publish_to_dlq(event: OutboxEvent, error: Exception):
    ...
    print(
        f"[Payment Outbox Publisher] Event sent to DLQ. "
        f"outbox_id={event.id} dlq_topic={DLQ_TOPIC}"
    )


def start_outbox_publisher():
    print("[Payment Outbox Publisher] Started...")
        ...
        try:
            ...
            for event in events:
                try:
                    ...
                    print(
                        f"[Payment Outbox Publisher] Published "
                    ...
                    if event.retry_count >= MAX_RETRY_COUNT:
                        try:
                            ...
                            print(
                                f"[Payment Outbox Publisher] DLQ publish failed. "
                        ...
                        print(
                            f"[Payment Outbox Publisher] Event moved to failed state. "
                            f"outbox_id={event.id} retry_count={event.retry_count}"
                        )

                    else:
                        print(
                            f"[Payment Outbox Publisher] Publish failed. "
            ...
            db.rollback()
            print(f"[Payment Outbox Publisher] Error: {error}")
